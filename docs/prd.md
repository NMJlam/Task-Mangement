...
| R8      | Budget                                    | Should   | 🟡  | Treasurer/Pres log expenses (desc/amount/category/date); lifecycle `pending→approved→paid`/`rejected`; per-event total + club summary (alloc/committed/spent cents, on-track/at-risk/critical). Others view-only. Schemas+risk ✅; treasurer UI + approval ⬜. Stripe = future.                                                                                                                                               |
